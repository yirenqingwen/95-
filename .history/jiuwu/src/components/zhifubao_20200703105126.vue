<template>
  <div class="zhifubao">
    <div class="cate_nav">
      <van-icon @click="fanhui" class="fanhui" name="arrow-left" />{{title}}
    </div>
    <div v-if="none" class="noneInfo">
      <div class="editInfo">
        <h3>收款账户信息</h3>
        <van-field placeholder="请输支付宝账号" v-model="zhifubao" />
        <van-field  placeholder="请输入姓名" v-model="realName" />
      </div>
      <div class="editInfo">
        <h3>发送验证码</h3>
        <van-field @input="input" placeholder="请输入验证码" v-model="sms">
          <template #button>
            <van-button size="small" type="primary">发送验证码</van-button>
          </template>
        </van-field>
      </div>
      <van-button @click="update" :disabled="disable" type="primary" block>认证信息</van-button>
    </div>
    <div v-if="!none" class="ali">
        <h3>我的支付宝账号</h3>
        <p v-text="login.zhifubao"></p><br><br>
        <van-button @click="update" :disabled="disable" type="primary" block>gang</van-button>
    </div>
  </div>
</template>

<script>
import { mapMutations } from "vuex";
export default {
  data() {
    return {
    title:"绑定收款账户",
      none: true,
      zhifubao: "",
      realName: "",
      sms:"",
      disable: true,
      login:{}
    };
  },
  methods: {
    ...mapMutations(["change"]),
    fanhui() {
      this.$router.go(-1);
      this.$store.commit("change", true);
    },
    input(value){
        if(this.sms.length==6){
            this.disable=false
        }else{
            this.disable=true
        }
    },
    update(){
        if(this.zhifubao!=""&&this.realName!=""){
            this.api.zhifubao({userId:this.$cookies.get("user_id"),
            realName:this.realName,zhifubao:this.zhifubao}).then((res)=>{
                this.$toast(res.data.msg)
            })
        }else{
            this.$toast("账号与姓名不能为空")
        }
    }
  },
  mounted() {
    this.api.searchLoginUser({userId:this.$cookies.get("user_id")}).then(
        (data)=>{
        this.login=data.data.data
          if(this.login.realName==""){
              this.none=true
          }else{
              this.title="我的支付宝账号"
              this.none=false
          }
        }
      )
    this.$store.commit("change", false);
  }
};
</script>

<style scoped>
.ali>p{
    padding: 1vh 0;
    color: black;
    border-bottom: 1px solid #f2f2f2;
}
.ali>h3{
    font-weight: 600;
    font-size: 14px;
}
.ali{
    padding: 2vh 4vw;
}
.editInfo > h3 {
  font-size: 14px;
  font-weight: 600;
}
.noneInfo /deep/.van-cell {
  padding: 2vh 0;
}
.editInfo {
  padding: 2vh 2vw;
}
.fanhui {
  float: left;
  font-size: 18px;
}
.zhifubao {
  padding: 0 4vw;
}
.cate_nav {
  color: black;
  width: 100%;
  padding: 2vh 2vw;
  font-size: 16px;
  font-weight: 600;
  text-align: center;
}
div {
  box-sizing: border-box;
}
</style>