<template>
  <div class="baseinfo">
    ...
    <div class="base_main">
      ...
      <div class="tab_btn">
          <van-button @click="confirmSale" :disabled="disable" type="primary" block>确认提交</van-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    jihuo2(data) {
      ...
      if(dom.className==`${data}`&&dom2.className==`${another}`){
        this.washType="none"
      }
    },
    confirmSale(){
      console.log("原始数据",this.baseData)
      let salTmp=this.baseData
      salTmp.salePerson=this.$cookies.get("user_id")
      salTmp.number=this.value
      salTmp.collect=0
      salTmp.fastSend=false
      salTmp.washType=this.washType
      salTmp.type="寄售"
      salTmp.id=this.baseData._id
      delete salTmp['_id']
      delete salTmp['__v']
      delete salTmp['menbao']
      console.log("新数据",salTmp)
      this.api.sale(salTmp).then(
        res=>{
