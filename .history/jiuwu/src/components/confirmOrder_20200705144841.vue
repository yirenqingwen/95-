  ...
  methods: {
    ...
    goaway(){
        ...
        let waitPay={
            ...
            money:(this.checked==false?this.orderData.price:this.orderData.price+29.9).toFixed(2),
            user:this.$cookies.get("user_id"),
            time:day.getFullYear()+"-"+month+"-"+day.getDate(),
            status:"待支付"
        }
        console.log(waitPay)
        this.api.lijibuy(waitPay).then(
          res=>{
            if(res.data.msg){
              this.$toast(res.data.msg)
              this.$router.push({name:"myBuy",params:{param:"待支付"}})
            }
          }
        )
    }
