    ...
    <van-action-sheet v-model="show" title="选择尺码">
      <div class="show_cont">
        <div class="every_size">
          <button class="size_btn" v-for="(i,index) in 30" :key="index" @click="filter($event,i*0.5+35)">{{i*0.5+35}}</button>
        </div>
        <van-button @click="next" type="primary" block>确认</van-button>
      </div>
    </van-action-sheet>
  </div>
</template>
