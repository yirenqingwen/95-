    ...
    <div class="person_main">
      <el-card class="box-card">
        <div slot="header" class="clearfix">
          ...
          <el-button style="float: right; padding: 3px 0;color:#ccc;" type="text">
            查看全部订单
            <van-icon name="arrow" />
          </el-button>
        </div>
  ...
  methods: {
    ...
    gotoSettings(){
      if(this.$cookies.get("user_id")){
        this.$router.push({name:"settings",params:{userinfo:this.userinfo}})
      }else{
        this.$router.push({name:"login"})
      }
    }
  ...
  mounted() {
    if(this.$cookies.get("user_id")){
      this.api.searchLoginUser({userId:this.$cookies.get("user_id")}).then(
        (data)=>{
          let login=data.data.data[0]
          if(data.data.data[0].nickName==""){
            this.userinfo={
              nickName:"95_"+data.data.data._id,
              photo:data.data.data.photo,
              phone:data.data.data.phone,
              realName:data.data.data.realName,
              zhifubao:data.data.data.zhifubao,
            }
          }else{
