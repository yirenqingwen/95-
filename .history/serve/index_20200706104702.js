...
app.post("/Sale", function (req, res) {
    console.log(req.body)
    let msg
    saleTable.find({ salePerson: req.body.salePerson, id: req.body.id }).exec(
        (err, data) => {
            if (data.length == 0) {
                ...
                saleTable.findOneAndUpdate({ salePerson: req.body.salePerson, id: req.body.id }, { number: req.body.number,washType:req.body.washType }).exec(
                    (err, data) => {
                        res.json({ msg: "添加成功" })
                    }
                )
