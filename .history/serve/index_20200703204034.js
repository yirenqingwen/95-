...
app.post("/xiugaiAddress",function(req,res){
    ...
    addresssTable.find({user:req.body.user}).exec(
        (err,data)=>{
            console.log(data[0].addressList)
            console.log(data[0].addressList.splice(req.body.newAddress.id,1))
            let newAddresList=data[0].addressList.push(req.body.newAddress)
            console.log("新数组",newAddresList)
            addresssTable.update({addressList:data[0].oldaddressList},{addressList:newAddresList}).exec(
                (err,data)=>{
                    console.log(data)
                }
            )
