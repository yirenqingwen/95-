...
// var mdday=myDay.getDate()
var noticeTable = allMongo.noticeTable
// var noticeItem=new noticeTable({
//     title:"1899抢开Air max 97万花筒!",
//     content:"更有AJ1影子、红丝绸、白蛇，YEEZY350 黑生胶,AJ11北卡蓝等你来开，12 点准时开始，抢先进场>>>",
//     time:mymonth+"-"+mdday,
//     link:"",
//     releasePerson:"官方"
// })
// noticeItem.save((err,data)=>{
//     console.log(data)
// })
// 浏览记录数据存储
var footprintTable = allMongo.footprintTable
// banner接口（无参数）
...
app.post("/getStatusData", function (req, res) {
    saleTable.find({ salePerson: req.body.user, status: req.body.status, saleType: req.body.saleType }).exec(
        (err, data) => {
            if (data.length == 0) {
                res.json({ msg: "暂无数据" })
            } else {
...
// 更新状态？
app.post("/updateSalestatus2", function (req, res) {
    let newStatus
    if (req.body.nowStatus == "出售中") {
        newStatus = "待发货"
    }
...
app.post("/footprint", (req, res) => {
    let flag = true
    footprintTable.find({ user: req.body.user }).exec(
        (err, data) => {
            data.forEach(item => {
                if (item.shangpin._id == req.body.shangpin._id) {
                    footprintTable.findByIdAndUpdate(item._id, { time: req.body.time }).exec(
                        (err, data) => {}
                    )
                    flag = false
                }
            ...
            if (flag == true) {
                var footprintItem = new footprintTable(req.body)
                footprintItem.save((err, data) => {})
            }
        }
    )
    res.json({ msg: "保存成功" })
...
// 查询浏览记录
app.post("/footprintCount",(req,res)=>{
    if(req.body.data){
        footprintTable.find({user:req.body.user}).exec(
            (err,data)=>{
                res.json({data:data})
        ...
        footprintTable.find(req.body).count(
            (err,data)=>{
                res.json({count:data})
            }
        )
...
// 清空浏览记录
app.post("/delatefootprint",(req,res)=>{
    console.log(req.body)
    footprintTable.remove({user:req.body.user}).exec(
        (err,data)=>{
            console.log(data)
            res.json({msg:"清除成功"})
        }
