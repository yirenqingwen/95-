...
app.post("/updateUser",photoConfig.single('PIC'),function(req,res){
    ...
    console.log(req.body)
    if(!req.body.PIC){
        userTable.updateMany(req.body.userId,{phone:req.body.TEL},
            {nickName:req.body.NICK},{photo:"http://localhost:3000/photo/"+req.file.filename}).exec(
                (err,data)=>{
                    console.log(data)
                    res.json({msg:"修改成功"})
                }
        ...
        userTable.findByIdAndUpdate(req.body.userId,{phone:req.body.TEL},
            {nickName:req.body.NICK}).exec(
                (err,data)=>{
                    console.log(data)
                    res.json({msg:"修改成功"})
                }
